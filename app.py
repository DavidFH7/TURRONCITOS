...
def eliminar_venta(venta_id):
    """Eliminar una venta individual y revertir el stock y ganancias asociadas"""
    try:
        venta = Venta.query.get_or_404(venta_id)

        # Revertir stock por cada producto en la venta
        items = db.session.execute(
            db.select(venta_producto.c.producto_id, venta_producto.c.cantidad)
              .where(venta_producto.c.venta_id == venta.id)
        ).all()

        for producto_id, cantidad in items:
            producto = Producto.query.get(producto_id)
            if producto and producto.stock:
                producto.stock.cantidad_disponible = (producto.stock.cantidad_disponible or 0) + int(cantidad or 0)

        # Eliminar ganancias asociadas a la venta
        Ganancias.query.filter_by(venta_id=venta.id).delete()

        # Eliminar items de la tabla intermedia
        db.session.execute(venta_producto.delete().where(venta_producto.c.venta_id == venta.id))
...
@app.route('/ventas/eliminar_todas', methods=['POST'])
@login_required
def eliminar_todas_ventas():
    """Eliminar todas las ventas y revertir stocks y ganancias"""
    try:
        # Revertir stock de todos los productos involucrados en ventas
        items = db.session.execute(
            db.select(venta_producto.c.producto_id, venta_producto.c.cantidad)
        ).all()

        # Sumar cantidades por producto para revertir en bloque
        cantidades_por_producto = {}
        for producto_id, cantidad in items:
            cantidades_por_producto[producto_id] = cantidades_por_producto.get(producto_id, 0) + int(cantidad or 0)

        for producto_id, cantidad_total in cantidades_por_producto.items():
            producto = Producto.query.get(producto_id)
            if producto and producto.stock:
                producto.stock.cantidad_disponible = (producto.stock.cantidad_disponible or 0) + cantidad_total

        # Eliminar ganancias, relaciones y ventas
        Ganancias.query.delete()
        db.session.execute(venta_producto.delete())
        db.session.query(Venta).delete()

        db.session.commit()
        flash('Todas las ventas fueron eliminadas y el stock fue revertido', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'Error al eliminar todas las ventas: {str(e)}', 'error')
    return redirect(url_for('ventas'))
...
def nueva_venta():
    ...
    if request.method == 'POST':
        ...
        venta = Venta(cliente_id=cliente_id, lugar_entrega_id=lugar_entrega_id, 
                     vendedor_id=vendedor_id, estado=estado, descuento_id=None, total=0)
...
@app.route('/respaldos/eliminar_todos', methods=['POST'])
@login_required
def eliminar_todos_respaldos():
    ...
    try:
        ...
        for backup_file in backup_files:
            try:
                ...
                flash(f'Error al eliminar {os.path.basename(backup_file)}: {str(e)}', 'error')
        
        if eliminados > 0:
            flash(f'Se eliminaron {eliminados} respaldos exitosamente', 'success')
        else:
            flash('No se pudo eliminar ningún respaldo', 'error')
            
    except Exception as e:
        flash(f'Error al eliminar respaldos: {str(e)}', 'error')
